...
module.exports.findReleases = async ({ app, context }) => {
  ...
  const sortedPrePublishedReleases = sortReleases(releases.filter((r) => r.prerelease))
  const draftRelease = releases.find((r) => r.draft)
  const lastPreRelease = sortedPrePublishedReleases[sortedPrePublishedReleases.length - 1]
  const lastRelease =
  ...
  if (lastPreRelease) {
    log({ app, context, message: `Last preRelease: ${lastPreRelease.tag_name}` })
  } else {
    log({ app, context, message: `No last preRelease found` })
  }

  return { draftRelease, lastRelease, lastPreRelease }
}
...
const getNoteFromBody = (body) => {
  ...
  const noteSection = entry.find(t => /^(\sNote)(\s)*(.*)/gu.test(t));
  if(noteSection) {
...
const generateChangeLog = (mergedPullRequests, config) => {
  ...
  const pullRequestToString = (pullRequests) =>
    pullRequests
      .map((pullRequest) => {
        const note = getNoteFromBody(pullRequest.body);
